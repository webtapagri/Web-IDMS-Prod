...
(SELECT SUBGROUP_DESCRIPTION FROM TM_SUBGROUP_ASSET WHERE JENIS_ASSET_CODE = a.JENIS_ASSET AND GROUP_CODE = a.GROUP AND SUBGROUP_CODE = a.SUB_GROUP) AS SUB_GROUP_NAME 
                    FROM TR_REG_ASSET_DETAIL a 
                        LEFT JOIN TM_GENERAL_DATA b ON a.BA_PEMILIK_ASSET = b.DESCRIPTION_CODE AND b.GENERAL_CODE = 'plant'
                    WHERE (a.KODE_ASSET_AMS IS NOT NULL OR KODE_ASSET_AMS != '' ) $where ORDER BY a.NO_REG DESC LIMIT ".$req['no-of-list']." ";
        //echo $sql; die();
